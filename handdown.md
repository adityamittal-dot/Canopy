...
| `languages/specs.py` | The 9 tree-sitter `TreeSitterLanguageSpec`s (JS, TS, Go, Java, Rust, C, C++, Ruby, PHP, C#) + registers the Python adapter. Node type/field names came from parsing real samples through each grammar during development (these PyPI packages ship no `node-types.json`), not from docs alone. |
